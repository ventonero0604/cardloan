<aside class="Feature_contents">
  <div class="Feature_contents_header">
    <h3 class="Feature_contents_title">
      <?php echo $GLOBALS['ranking1stData']['serviceName'] ?>
    </h3>

    <?php if ($GLOBALS['displayStar'] === true): ?>
      <div class="Feature_contents_rating">
        ...
        <p class="rate js-rate"><?php echo $GLOBALS['ranking1stScore'] ?></p>
      </div>
    <?php else: ?>
      <div class="Feature_contents_value">
        <p class="title">申込実績</p>
        <p class="value js-inview-value">
          <em><?php echo $GLOBALS['ranking1stData']['users'] ?></em>件
        </p>
      </div>
    <?php endif; ?>
  </div>
  ...
  <div class="Feature_contents_body">
    <?php require __DIR__ . '/labels.php'; ?>
    <div class="Feature_contents_main">
      <a href="<?php echo $GLOBALS['ranking1stData']['url'] ?>" class="thumb">
        <img src="images/thumb/<?php echo $GLOBALS['ranking1stData']['banner'] ?>" alt="">
      </a>
      <table>
        <tbody>
          <tr>
            <th>審査時間</th>
            <td><?php echo $GLOBALS['ranking1stData']['strExamTime'] ?></td>
          </tr>
          <tr>
            <th>融資時間</th>
            <td><?php echo $GLOBALS['ranking1stData']['strLoanTime'] ?></td>
          </tr>
          <tr>
            <th>実質年率</th>
            <td><?php echo $GLOBALS['ranking1stData']['minRate'] ?>% ~<?php echo $GLOBALS['ranking1stData']['maxRate'] ?>%</td>
          </tr>
          <tr>
            <th>収入証明書</th>
            <td><?php echo $GLOBALS['ranking1stData']['strConfirmation'] ?></td>
          </tr>
        </tbody>
      </table>
    </div>
    <?php if (!empty($GLOBALS['ranking1stData']['shortComment'])): ?>
      <div class="Feature_contents_info">
        ...
        <div class="texts">
          <p class="title">
            <em><?php echo $GLOBALS['ranking1stData']['serviceName'] ?></em><span>を</span>選ぶ理由
          </p>
          <p class="text">
            <?php echo $GLOBALS['ranking1stData']['shortComment']; ?>
          </p>
        </div>
      </div>
    <?php endif; ?>
  ...
  <div class="Feature_footer">
    <?php if ($GLOBALS['ranking1stData']['flagTimer'] === true): ?>
    ...
    <?php endif; ?>
    <a href="<?php echo $GLOBALS['ranking1stData']['url'] ?>" class="Button">
      【公式】詳しくはこちら
    </a>
  </div>
</aside>